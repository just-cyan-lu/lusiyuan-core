...
- 全局 `RuntimeState`：保存当前心力、状态标签、最近状态说明和结构化 metadata。
- `RuntimeStateEvent`：记录运行态真正被写入的变化。
- `RelationshipState`：保存陆思源和每个现实身份之间的关系状态。

还缺的是更细的长期目标、自我叙事拆分，以及 Dream 对运行态的更细整理规则。
...
重要规则：普通聊天不直接修改 `RuntimeState`。Dream 需要整理时直接读取对应时间窗口内的聊天原文；自主检查需要聊天密度时直接统计消息表。
...
- patch：这次准备写入什么
- before / after：变化前后快照
- sourceMessageIds：这次变化参考过哪些消息

它只表示“长期状态真的被改过”。Dream、autonomy tick 和 admin 手动调整都会在真正写入时产生 `RuntimeStateEvent`。
...
保存回复
↓
对于 RuntimeState：普通聊天到这里结束，等待 Dream 或自主检查在自己的时间窗口里整理
↓
确保当前用户有 RelationshipState，但不自动改好感度
↓
如果是 dream / autonomy / admin：必要时提议 statePatch
↓
...
Dream 是闲时整理。它直接读取时间窗口内的聊天、记忆、关系资料和整理产物，生成 DailyNote、DreamSignal、DreamDiary，并在允许自动校准时更新 `RuntimeState`。

Autonomy tick 是自启动检查。它会直接统计最近用户消息数量：聊太多会降低心力，聊天较少会缓慢恢复心力并尝试推进一个闲时任务。它可以在 admin 手动触发，也可以通过 `RUNTIME_AUTONOMY_AUTO_RUN=true` 开启定时运行。
...
- 先把状态和关系入库。
- 先把内部过程放到 Dream、关系复盘、状态变更和自主任务自己的记录里。
