...
admin 里的运行事件详情只解释这条事件本身，以及它和最近状态变化有没有关系。

**RuntimeStateEvent**

运行态变化记录。比如 owner 对话校准、复盘更新、梦境更新、自启动更新、手动调整、重置。它保存变化摘要、patch、变化前后快照和来源信息。

一次状态变化可能来自很多材料，不一定只来自一条消息。所以它会保存两层来源：`sourceRuntimeEventIds` 记录哪些运行事件参与了这次变化，`sourceMessageIds` 记录背后的多条消息。就算只有一条消息，也统一放进 `sourceMessageIds`。
