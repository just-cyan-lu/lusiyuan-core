...
每个现实身份有一份 `RelationshipState`，记录单一 `affinity`（好感度）、互动风格、关系摘要和最近信号。

**为什么这样做**

这和全局 `RuntimeState` 不一样。`RuntimeState` 是“陆思源整体现在怎么样”，`RelationshipState` 是“陆思源和这个人现在是什么关系”。

所以普通用户的聊天不应该乱改陆思源整体心情；关系变化也不再由单轮聊天关键词直接决定。

**现在怎么变化**

当前关系状态只保留一条好感度主线：

1. 聊天只确保当前用户有一份关系状态，不自动升降好感度。
2. admin 可以在关系页手动修正好感度、互动风格、关系摘要等字段。
3. Reflection/Dream 后续如果根据证据判断要调整关系，应调用 `relationshipStateService.applyAffinityPatch(...)` 写入完整原因和证据。
4. 好感度变化会写 `affinity_update` 事件，方便审计、导出和后续训练数据整理。

**为什么不用每轮聊天直接改**

因为人和人的关系不应该被某一句话或几个关键词马上拉高或拉低。现在的设计是：聊天提供材料，后续复盘或梦境在更完整的上下文里决定关系变化。
