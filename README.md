...
这个项目是陆思源的"大脑后台"。用户不管通过什么方式和他聊天（Telegram、网页、未来可能的其他平台），消息最终都会进入这个后台处理，再把回复发回去。
...
⑤ 组装提示词（Prompt）
   ...
   └─────────────────────────────────┘
        ...
        ▼
⑥ 调用 AI 模型
   发给 OpenAI 兼容接口（可以是 GPT、Qwen 等任何模型）
   模型返回陆思源的回复文本
        │
        ▼
⑦ 工具调用（可选，默认关闭）
   如果开启了工具功能，模型可以在回复前
   先查询一些内部数据（比如项目状态、历史记录）
   再生成更准确的回复
        │
        ▼
⑧ 清理输出
   去掉模型可能带出来的格式垃圾（比如 <think> 标签）
        │
        ▼
⑨ 存储回复 + 提取记忆（后台异步）
   把陆思源的回复存进数据库
   同时在后台悄悄分析这段对话，提取值得记住的信息
   比如"用户说他叫小明" → 写入长期记忆
        │
        ▼
⑩ 返回回复
   把回复文本发回给 Telegram 或网页前端
   用户看到陆思源的消息
```
...
| 你看到的 | 对应的代码位置 |
|---------|--------------|
| 陆思源的性格、说话方式 | `persona/` 文件夹里的 Markdown |
| 消息处理核心流程 | `src/core/chat.service.ts` |
| 长期记忆的读写 | `src/core/memory.service.ts` |
| 发给模型的提示词怎么拼 | `src/core/prompt-builder.ts` |
| Telegram 接入 | `src/channels/telegram/` |
