...
旧版 README 已备份为 `README.legacy-2026-06-30.md`。当前 README 先只记录已经重新梳理过的主线：聊天上下文、运行态与自主活动、关系复盘与关系档案。其他功能等逐个优化后再补。
...
3. **关系复盘与关系档案**：陆思源怎样把不同渠道账号归到同一个现实身份，并通过 Dream 维护关系资料。

这三个模块分别负责“接住对话”、“有自己的连续生活感”和“慢慢认识每个真实互动对象”。它们是目前 README 里优先整理的主线。
...
新的边界是：关系不再靠单轮聊天关键词自动升温，也不再拆成多维数值。每个现实身份只有一份关系档案，Dream 会在一段时间后做整体关系复盘，再决定要不要更新档案。
...
它会按现实身份分组读取这次 Dream 时间窗口内的聊天，结合当前关系档案和历史证据，生成一次整体复盘。
...
关系页里的“关系变更”现在按时间线展示，不再按类型筛选。点开一条记录后，右侧会展示实际改动和原始 JSON。
