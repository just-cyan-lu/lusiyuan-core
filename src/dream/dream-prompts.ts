...
请根据 DreamSignals 和 DailyNote，判断哪些全局、项目或话题内容值得直接写入长期记忆。
...
要求：
1. 只输出真正值得长期保留的全局、项目或话题记忆变化。
2. 每条变化必须引用原始 sourceIds（sourceMessageIds 字段）。
3. DreamDiary 不能作为唯一证据。
4. 装真人风险只能生成 riskFlag，不能生成"陆思源是真人"的记忆。
5. 不要生成个人记忆、关系记忆或相处模式记忆；这些由 Relationship Review 按身份整理。
6. scope 只能是 project、global 或 topic。global 会影响所有用户，必须非常少且有充分证据。
7. 证据不足或只是暂时热点时，放入 openQuestions，不要硬写记忆。
