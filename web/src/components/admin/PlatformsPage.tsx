...
function statusClass(status: PlatformStatus): string {
  if (status === "ready") return "border-[var(--ls-success-border)] bg-[var(--ls-success-bg)] text-[var(--ls-success-text)]";
  if (status === "planning") return "border-[var(--ls-border)] bg-[var(--ls-panel-soft)] text-[var(--ls-ink-soft)]";
  return "border-[var(--ls-info-border)] bg-[var(--ls-panel-soft)] text-[var(--ls-info-text)]";
}
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      <section className="rounded-lg border border-[var(--ls-border)] bg-white p-6 shadow-[var(--ls-shadow)] md:p-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-xs font-semibold text-[var(--ls-eyebrow-text)]">Platform Directory</div>
            <h2 className="mt-2 text-3xl font-semibold text-[var(--ls-ink-strong)]">平台工作台</h2>
...
function PlatformCard({
  ...
  return (
    <article className="mb-5 inline-block w-full break-inside-avoid overflow-hidden rounded-lg border border-[var(--ls-border)] bg-white shadow-sm">
      <div className="h-1.5" style={{ backgroundColor: platform.accent }} />
      <div className={tall ? "p-5 md:p-6" : "p-5"}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-xs text-[var(--ls-ink-soft)]">{platform.subtitle}</div>
            <h3 className="mt-1 text-2xl font-semibold text-[var(--ls-ink-strong)]">{platform.name}</h3>
          </div>
        ...
        <p className="mt-4 text-sm leading-7 text-[var(--ls-ink-soft)]">{platform.description}</p>

        <div className="mt-5 grid grid-cols-3 gap-2">
          {platform.metrics.map((metric) => (
            <div key={metric.label} className="rounded-lg border border-[var(--ls-border)] bg-[var(--ls-panel-soft)] px-3 py-2">
              <div className="text-[11px] text-[var(--ls-ink-soft)]">{metric.label}</div>
              <div className="mt-1 truncate text-sm font-semibold text-[var(--ls-ink-strong)]">{metric.value}</div>
            </div>
          ))}
        </div>
        ...
        {available ? (
          ...
          <div className="mt-5 flex h-10 w-full items-center justify-center rounded-lg border border-[var(--ls-border)] bg-[var(--ls-panel-soft)] px-4 text-sm font-medium text-[var(--ls-ink-soft)]">
            详情待规划
          </div>
        )}
      </div>
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      <section className="rounded-lg border border-[var(--ls-border)] bg-white p-6 shadow-[var(--ls-shadow)] md:p-7">
        ...
        <div className="mt-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-xs font-semibold text-[var(--ls-eyebrow-text)]">Platform / Xiaohongshu</div>
            <h2 className="mt-2 text-3xl font-semibold text-[var(--ls-ink-strong)]">思源的小红书</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--ls-ink-soft)]">
        ...
        {importWarning && (
          <div className="mt-5 rounded-lg border border-[var(--ls-info-border)] bg-[var(--ls-panel-soft)] px-4 py-3 text-sm leading-6 text-[var(--ls-info-text)]">
            {importWarning}
          </div>
        )}
      </section>
        ...
        <div className="space-y-5">
          <SectionPanel title="从小红书读取" subtitle="粘贴帖子 URL，读取当前页面已经加载的内容">
            <div className="grid gap-3">
              ...
              <p className="text-xs leading-6 text-[var(--ls-ink-soft)]">
          ...
          <SectionPanel title="帖子列表" subtitle={`${posts.length} 条帖子`}>
            <div className="grid gap-2">
              {posts.length > 0 ? posts.map((post) => (
                ...
                >
                  <div className="text-sm font-semibold text-[var(--ls-ink-strong)]">{post.title}</div>
...
function PostRecordEditor({
  ...
  return (
    <section className="admin-select-host border-b border-[var(--ls-border)] pb-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="text-xs font-semibold text-[var(--ls-eyebrow-text)]">已读取的帖子内容</div>
          {post.url && (
            <a href={post.url} target="_blank" rel="noreferrer" className="mt-1 block max-w-xl truncate text-xs text-[var(--ls-link)] underline">
              {post.url}
            </a>
          )}
        </div>
      ...
      <label className="mt-3 block">
        <span className="mb-1 block text-xs font-semibold text-[var(--ls-ink-soft)]">帖子文案</span>
      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-[var(--ls-border)] bg-[var(--ls-panel-soft)]">
        <button
          type="button"
        ...
        >
          <span>
            <span className="block text-xs font-semibold text-[var(--ls-ink-soft)]">配图 Alt</span>
            <span className="mt-1 block text-xs text-[var(--ls-ink-soft)]">{alts.length} 个位置 · {filledAltCount} 个已填写</span>
          </span>
          <span className="shrink-0 rounded-md border border-[var(--ls-border)] bg-white px-2.5 py-1 text-xs font-medium text-[var(--ls-link)]">
            {altsOpen ? "收起" : "展开"}
          </span>
        </button>
        {altsOpen && (
          <div className="border-t border-[var(--ls-border)] bg-white px-3 py-3">
            <div className="grid gap-2">
              {alts.length > 0 ? alts.map((alt, index) => (
                <div key={index} className="grid gap-2 sm:grid-cols-[4rem_1fr_auto] sm:items-center">
                  <label htmlFor={`post-alt-${post.id}-${index}`} className="text-xs text-[var(--ls-ink-soft)]">
                    第 {index + 1} 张
                  </label>
                  <AdminInput
                ...
                <div className="rounded-lg border border-dashed border-[var(--ls-border)] bg-[var(--ls-panel-soft)] px-3 py-4 text-xs text-[var(--ls-ink-soft)]">
                  暂无配图 Alt 位置
                </div>
              )}
            </div>
...
function CommentThreadBlock({
  ...
  return (
    <article className={`rounded-lg border bg-white ${activeComment ? "border-[var(--ls-border-cold-soft)]" : "border-[var(--ls-border)]"}`}>
      <div className="p-4">
        <CommentSummary
          comment={thread}
        ...
        {thread.replies.length > 0 && (
          <div className="ml-3 mt-3 border-l-2 border-[var(--ls-border-cold)] pl-4">
            {thread.replies.map((reply) => (
              <div key={reply.id} className="border-t border-[var(--ls-border)] py-3 first:border-t-0 first:pt-0 last:pb-0">
                <CommentSummary
                  comment={reply}
  ...
  return (
    <button type="button" onClick={onSelect} className={`admin-layout-button -mx-2 block w-[calc(100%+1rem)] rounded-md px-2 py-1 text-left transition-colors hover:bg-[var(--ls-panel-cold)] ${active ? "is-active" : ""}`}>
      <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--ls-ink-soft)]">
        ...
        {comment.isAuthor && (
          <span className="border border-[var(--ls-success-bg-soft)] bg-[var(--ls-panel-cold)] px-1.5 py-0.5 text-[var(--ls-success-text-strong)]">作者</span>
        )}
  ...
  return (
    <div className="border-t border-[var(--ls-border)] bg-[var(--ls-panel-soft)] px-4 py-4">
      ...
      {comment.learningExample && (
        <div className="mt-3 border-l-2 border-[var(--ls-link-soft)] bg-[var(--ls-panel-cold)] px-4 py-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      {latestDraft && (
        <div className="mt-3 rounded-lg border border-[var(--ls-border)] bg-white p-3">
          <div className="mb-2 flex flex-wrap gap-2">
            <span className="rounded-full border border-[var(--ls-border)] bg-[var(--ls-panel-soft)] px-2.5 py-1 text-xs text-[var(--ls-ink-soft)]">
          ...
          {latestDraft.risk === "skip" ? (
            <div className="text-sm leading-7 text-[var(--ls-ink-soft)]">建议不回复。</div>
          ) : (
            <DraftEditor
              key={latestDraft.id}
              draft={latestDraft}
          ...
          {latestDraft.reason && (
            <div className="mt-2 text-xs leading-5 text-[var(--ls-ink-soft)]">{latestDraft.reason}</div>
          )}
        </div>
      )}
  ...
  return (
    <div>
      ...
      {draft.originalContent && draft.originalContent !== content && (
        <details className="mt-2 text-xs text-[var(--ls-ink-soft)]">
          <summary className="cursor-pointer">查看思源最初生成的版本</summary>
          <div className="mt-2 border-l-2 border-[var(--ls-border)] pl-3 leading-6">{draft.originalContent}</div>
        </details>
      )}
      <label className="mt-3 block">
        <span className="mb-1 block text-xs font-semibold text-[var(--ls-ink-soft)]">你为什么这样改（可不填）</span>
        <textarea
          value={ownerNote}
...
function OwnerReplyRecorder({
  ...
  return (
    <div className="mt-3 border-t border-[var(--ls-border)] pt-3">
      <div className="text-xs font-semibold text-[var(--ls-ink-soft)]">记录你在真实小红书上的最终处理</div>
      <textarea
        value={content}
        onChange={(event) => setContent(event.target.value)}
...
function EmptyBlock({ children }: { children: ReactNode }) {
  return (
    <div className="rounded-lg border border-dashed border-[var(--ls-border)] bg-[var(--ls-panel-soft)] px-4 py-6 text-sm leading-6 text-[var(--ls-ink-soft)]">
      {children}
    </div>
  );
}
