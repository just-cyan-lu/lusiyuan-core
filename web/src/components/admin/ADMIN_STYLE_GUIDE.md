...
- Admin 外壳使用 `Cursor forceAll={false}`，保留输入框 text cursor、按钮 pointer、禁用 not-allowed。
...
- `Cursor forceAll={false}`：自定义手指光标；保留输入框 text cursor、交互元素 pointer、禁用态 not-allowed。
