...
陆思源聊天时不是简单读取“最近 10 条消息”。

现在的逻辑更像给模型准备一个分层资料包，让它既能接住刚刚发生的事，也能在需要时想起更早聊过的内容。

### 第一层：最近原文热区

这是最直接的上下文。

系统会从当前对话里取最近的原文聊天，但不是按条数取，而是按字符预算取。这个预算由 `CHAT_CONTEXT_RECENT_MAX_CHARS` 控制。

读取时会做一些整理：

- 过滤掉工具调用前那类临时中间消息。
- 如果陆思源一次回复被拆成多条气泡，会合并回一条完整回复。
...
如果一段对话已经离现在比较远，系统不会一直把原文全部塞进 prompt，而是会在后台把它压缩成摘要，存到 `conversation_context_summaries`。

摘要会保留来源范围，比如从哪条消息到哪条消息、时间范围、消息数量。这样以后要审计、导出或重新生成都还有依据。

这层负责保留更早的事实、约定、偏好和未完成事项，但减少噪声。

### 第三层：相关旧原文窗口

如果开启 `CHAT_CONTEXT_RECALL_ENABLED`，系统会把当前问题做向量检索，从历史消息里找到语义相关的旧消息。

命中后，它不是只给模型一句“这里可能聊过”，而是把命中消息前后的一小段原文窗口放进 prompt。这样当用户问“我们以前是不是说过这个”时，陆思源能看到当时附近的真实对话。
...
最外层还有 `CHAT_CONTEXT_MAX_CHARS`，控制“最近原文 + 较早摘要 + 旧原文召回”加起来最多放多少内容。

目前这里用的是字符预算，不是精确 token 预算。之后如果接入 tokenizer，可以升级成更精确的 token 预算。

对应代码：

- `src/core/conversation-context.service.ts`：三层上下文的总调度。
- `src/core/chat-context.ts`：最近原文热区的整理和裁剪。
